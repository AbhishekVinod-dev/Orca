...
async def call_agent(
    llm, prompt: str, role: str, SYSTEM_PROMPT: str, available_tools: dict = None, agent_name: str = "agent"
) -> str:
    ...
    while True:
        ...
        if "<tool_call>" in curr_msg.content:
            try:
                ...
                if available_tools and tool_name in available_tools:
                    ...
                    import inspect
                    if inspect.isasyncgenfunction(tool_function) or (hasattr(tool_function, '__name__') and 'call_' in tool_function.__name__):
                        sub_agent_final_result = ""
                        async for chunk in tool_function(prompt=tool_args.get("prompt", ""), role=role):
                            if '"type": "final"' in chunk:
                                chunk_data = chunk.replace("data: ", "").strip()
                                sub_agent_final_result = json.loads(chunk_data).get("content", "")
                            else:
                                yield chunk
                    ...
                    tool_result_content = f"Error: Tool {tool_name} is not available to this agent."
                
                messages.append(
                    HumanMessage(content=f"<tool_result>{tool_result_content}</tool_result>")
                )
        ...
        if "<final>" in curr_msg.content:
            try:
                final_answer = curr_msg.content.split("<final>")[1].split("</final>")[0].strip()
